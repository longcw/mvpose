...
def create_global_net(blocks, is_training, trainable=True):
    ...
    for i, block in enumerate(reversed(blocks)):
        ...
        if last_fm is not None:
            sz = tf.shape(lateral)
            upsample = tf.image.resize_bilinear(last_fm, (sz[1], sz[2]), name='upsample/res{}'.format(5 - i))
            upsample = slim.conv2d(
                upsample,
                256,
                [1, 1],
                trainable=trainable,
...
def create_refine_net(blocks, is_training, trainable=True):
    ...
    for i, block in enumerate(blocks):
        ...
        with slim.arg_scope(resnet_arg_scope(bn_is_training=is_training)):
            for j in range(i):
                mid_fm = bottleneck(
                    mid_fm, 256, 128, stride=1, scope='res{}/refine_conv{}'.format(2 + i, j)
                )  # no projection
        mid_fm = tf.image.resize_bilinear(
            mid_fm, (cfg.output_shape[0], cfg.output_shape[1]), name='upsample_conv/res{}'.format(2 + i)
        )
        ...
        if is_train:
            ...
            def ohkm(loss, top_k):
                ...
                for i in range(cfg.batch_size):
                    ...
                    tmp_loss = tf.gather(
                        sub_loss, topk_idx, name='ohkm_loss{}'.format(i)
                    )  # can be ignore ???
            ...
            for i, (global_out, label) in enumerate(zip(global_outs, labels)):
                global_label = label * tf.to_float(
                    tf.greater(tf.reshape(valids, (-1, 1, 1, cfg.nr_skeleton)), 1.1)
                )

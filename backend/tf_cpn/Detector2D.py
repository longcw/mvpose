...
class Detector2D(object):
    def __init__(
        self, test_model=os.path.join(cfg.cur_dir, 'log/model_dump/snapshot_350.ckpt'), show_image=False
    ):
            ...
            for test_id in range(0, len(test_data), batch_size):
                ...
                for test_image_id in range(start_id, end_id):
                    ...
                    def test_for_loop(raw_heatmaps, order=3):
                        ...
                        for w in range(cfg.nr_skeleton):
                            heatmaps_this[w] = scipy.ndimage.interpolation.zoom(
                                raw_heatmaps[w], (width / l_ori, length / w_ori), order=order
                            )
                    ...
                    def test_my(raw_heatmaps, order=3):
                        ...
                        zoomed_heatmaps = scipy.ndimage.interpolation.zoom(
                            origin_heatmap, (1, width / l_ori, length / w_ori), order=order
                        )
                    ...
                    for zoom_id, heatmap_i in enumerate(raw_heatmaps):
                        zoomed_heatmaps[zoom_id] = scipy.ndimage.interpolation.zoom(
                            heatmap_i, (width / l_ori, length / w_ori), order=3
                        )

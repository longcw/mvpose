...
from detection_opr.rpn_batched.anchor_target_layer_without_boxweight import anchor_target_layer
...
class Network(object):
    ...
    def inference(self, mode, inputs):
        ...
        blocks = [
            resnet_utils.Block('block1', bottleneck, [(256, 64, 1, 1)] * 2 + [(256, 64, 1, 1)]),
            resnet_utils.Block('block2', bottleneck, [(512, 128, 2, 1)] + [(512, 128, 1, 1)] * 3),
            resnet_utils.Block('block3', bottleneck, [(1024, 256, 2, 1)] + [(1024, 256, 1, 1)] * 22),
            resnet_utils.Block('block4', bottleneck, [(2048, 512, 1, 2)] + [(2048, 512, 1, 2)] * 2),
        ]
        ...
        with slim.arg_scope(resnet_arg_scope(is_training=False)):
            ...
            net, _ = resnet_v1.resnet_v1(
                net, blocks[0:1], global_pool=False, include_root_block=False, scope='light_resnet_v1_101'
            )

        with slim.arg_scope(resnet_arg_scope(is_training=is_training)):
            net_conv3, _ = resnet_v1.resnet_v1(
                net, blocks[1:2], global_pool=False, include_root_block=False, scope='light_resnet_v1_101'
            )
        ...
        with tf.variable_scope(
            'light_resnet_v1_101',
            'light_resnet_v1_101',
            regularizer=tf.contrib.layers.l2_regularizer(cfg.weight_decay),
        ):
            # rpn
            rpn = slim.conv2d(
                net_conv4,
            ...
            anchors = generate_anchors_opr(height, width, cfg.stride[0], cfg.anchor_scales, cfg.anchor_ratios)
            ...
            if is_training:
                with tf.variable_scope('anchor') as scope:
                    rpn_labels, rpn_bbox_targets = tf.py_func(
                        anchor_target_layer,
                        [gt_boxes, im_info, cfg.stride, anchors, num_anchors],
        ...
        with tf.variable_scope(
            ...
            ps_fm = rfcn_plus_plus_opr.global_context_module(
                net_conv5, prefix='conv_new_1', ks=15, chl_mid=256, chl_out=ps_chl
            )
            ps_fm = nn_ops.relu(ps_fm)

            [psroipooled_rois, _, _] = psalign_pooling_op.psalign_pool(
                ps_fm, rois, group_size=7, sample_height=2, sample_width=2, spatial_scale=1.0 / 16.0
            )
        ...
        if not is_training:
            stds = np.tile(np.array(cfg.TRAIN.BBOX_NORMALIZE_STDS), (cfg.num_classes))
            means = np.tile(np.array(cfg.TRAIN.BBOX_NORMALIZE_MEANS), (cfg.num_classes))
            bbox_pred *= stds
            bbox_pred += means
            ...
            cross_entropy, loss_box = loss_opr_without_box_weight.sum_ohem_loss(
                tf.reshape(cls_score, [-1, cfg.num_classes]),
                label,
                bbox_pred,
                bbox_targets,
                cfg.TRAIN.nr_ohem_sampling,
                cfg.num_classes,
            )
            loss_box *= 2

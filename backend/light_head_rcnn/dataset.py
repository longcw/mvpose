...
def get_hw_by_short_size(im_height, im_width, short_size, max_size):
    ...
    resized_height, resized_width = int(round(im_height * scale)), int(round(im_width * scale))
    return resized_height, resized_width


def resize_img_by_short_and_max_size(img, short_size, max_size, *, random_scale_methods=False):
    resized_height, resized_width = get_hw_by_short_size(img.shape[0], img.shape[1], short_size, max_size)
    ...
    img = cv2.resize(img, (resized_width, resized_height), interpolation=chosen_resize_option)
    return img, scale
...
def pad_image_and_boxes(img, height, width, mean_value, boxes=None):
...
def get_data_for_singlegpu(batch_lines):
    ...
    batch_images = np.zeros((batch_per_gpu, batch_resized_height, batch_resized_width, 3), dtype=np.float32)
    batch_gts = np.zeros((batch_per_gpu, config.max_boxes_of_image, config.nr_box_dim), dtype=np.float32)
    ...
    for i in range(batch_per_gpu):
        ...
        for j, gb in enumerate(gt_boxes):
            if gb.ign != 1 or not config.filter_gt_ignore_label:
                gt[gt_idx, :] = [gb.x, gb.y, gb.x1, gb.y1, config.class_names.index(gb.tag)]
                gt_idx += 1
            elif config.train_gt_ignore_label:
                gt[gt_idx, :] = [gb.x, gb.y, gb.x1, gb.y1, config.anchor_ignore_label]
                gt_idx += 1
        ...
        padded_image, padded_gt = pad_image_and_boxes(
            img, batch_image_height, batch_image_width, config.image_mean, gt
        )
        ...
        resized_image, scale = resize_img_by_short_and_max_size(padded_image, short_size, max_size)
        padded_gt[:, 0:4] *= scale
        ...
        if np.random.randint(2) == 1:
            resized_image, resized_gt = flip_image_and_boxes(resized_image, resized_gt)
    ...
    return dict(data=batch_images, boxes=batch_gts, im_info=batch_info, is_valid=is_batch_ok)
    # return dict(
...
def val_dataset():
    ...
    def read_func(line):
        ...
        # ground truth
        img = cv2.imread(image_path, cv2.IMREAD_COLOR)
        return dict(data=img, boxes=gt_boxes, image_id=image_id, image_path=image_path)

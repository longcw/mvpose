...
class PersonDetector(object):
    ...
    def __init__(self, epoch_num=26, show_image=False):
        ...
        self.model_file = osp.join(config.output_dir, 'model_dump', 'epoch_{:d}'.format(epoch_num) + '.ckpt')
    ...
    def inference(self, val_func, inputs, data_dict):
        ...
        im_info = np.array([[height, width, scale, ori_shape[0], ori_shape[1], 0]], dtype=np.float32)
        ...
        for j in range(1, config.num_classes):
            ...
            cls_dets = np.hstack((cls_bboxes, cls_scores[:, np.newaxis])).astype(np.float32, copy=False)
            ...
            for i in range(cls_dets.shape[0]):
                db = cls_dets[i, :]
                dbox = DetBox(
                    db[0], db[1], db[2] - db[0], db[3] - db[1], tag=config.class_names[j], score=db[-1]
                )
                result_boxes.append(dbox)
        if len(result_boxes) > config.test_max_boxes_per_image:
            result_boxes = sorted(result_boxes, reverse=True, key=lambda t_res: t_res.score)[
            ...
            if config.test_save_type == 'coco':
                ...
                for rb in result_boxes:
                    if rb.tag == 'person':
                        record = {
                            'image_id': image_id,
                            'category_id': config.datadb.classes_originID[rb.tag],
                            'score': rb.score,
                            'bbox': [rb.x, rb.y, rb.w, rb.h],
                            'data': result['data'],
                        }
                        coco_records.append(record)
            else:
                raise Exception("Unimplemented save type: " + str(config.test_save_type))

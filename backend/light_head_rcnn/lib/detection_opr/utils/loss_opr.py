...
def smooth_l1_loss(bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights, sigma=1.0, dim=[1]):
    value = _smooth_l1_loss_base(
        bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights, sigma=sigma, dim=[1]
    )
    loss = tf.reduce_mean(tf.reduce_sum(value, axis=dim))
    return loss


def smooth_l1_loss_ohem(
    bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights, nr_ohem_sampling, sigma=1.0, dim=[1]
):
...
def debug_four(cls_loss, box_loss, topk_idx, cls_loss_ohem, box_loss_ohem, cls_norm):
    from IPython import embed
...
def smooth_l1_loss_valid(
    ...
    value = _smooth_l1_loss_base(
        bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights, sigma, dim=[1]
    )

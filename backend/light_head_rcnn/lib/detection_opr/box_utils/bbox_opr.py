...
def _concat_new_axis(t1, t2, t3, t4, axis=1):
    return Concat([t1.add_axis(-1), t2.add_axis(-1), t3.add_axis(-1), t4.add_axis(-1)], axis=axis)
...
def _box_ltrb_to_cs_opr(bbox, addaxis=None):
    ...
    if addaxis is None:
        ...
        return (
            bbox_width.add_axis(addaxis),
            bbox_height.add_axis(addaxis),
            bbox_ctr_x.add_axis(addaxis),
...
def clip_boxes_opr(boxes, im_info):
    ...
    clip_box = _concat_new_axis(box_x1, box_y1, box_x2, box_y2, 2).reshape(boxes.shape[0], -1)
...
def filter_anchors_opr(all_anchors, im_height, im_width, allowed_border_height, allowed_border_width=None):
    if allowed_border_width is None:
        allowed_border_width = allowed_border_height
    inds_inside = (
        (all_anchors[:, 0] >= -allowed_border_width)
...
def bbox_transform_inv_opr(anchors, deltas):
    ...
    anchor_width, anchor_height, anchor_ctr_x, anchor_ctr_y = _box_ltrb_to_cs_opr(anchors, 1)
...
def box_overlap_opr(box, gt):
    ...
    iw = Min(b_box[:, :, 2], b_gt[:, :, 2]) - Max(b_box[:, :, 0], b_gt[:, :, 0]) + 1
    ih = Min(b_box[:, :, 3], b_gt[:, :, 3]) - Max(b_box[:, :, 1], b_gt[:, :, 1]) + 1
...
def box_overlap_ignore_opr(box, gt, *, ignore_label=-1, return_separate=False):
    ...
    gt_ignore_mask = gt[:, 4].eq(ignore_label).add_axis(0).broadcast(area_target_shape)

...
def generate_anchors_pre(
    height, width, feat_stride, anchor_scales=(8, 16, 32), anchor_ratios=(0.5, 1, 2), base_size=16
):
    """ A wrapper function to generate anchors given different scales
      Also return the number of anchors in variable 'length'
    """
    anchors = generate_anchors(
        ratios=np.array(anchor_ratios), scales=np.array(anchor_scales), base_size=base_size
    )
    A = anchors.shape[0]
...
def generate_anchors_opr(
    ...
    shifts = tf.stack(
        (
            tf.reshape(shift_x, (-1, 1)),
            tf.reshape(shift_y, (-1, 1)),
            tf.reshape(shift_x, (-1, 1)),
            tf.reshape(shift_y, (-1, 1)),
        )
    )
    shifts = tf.transpose(shifts, [1, 0, 2])
    final_anc = tf.constant(anchors.reshape((1, -1, 4)), dtype=np.float32) + tf.transpose(
        tf.reshape(shifts, (1, -1, 4)), (1, 0, 2)
    )

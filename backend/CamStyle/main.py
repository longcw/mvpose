...
def get_data(dataname, data_dir, height, width, batch_size, camstyle=0, re=0, workers=8):
    ...
    train_loader = DataLoader(
        Preprocessor(
            dataset.train, root=osp.join(dataset.images_dir, dataset.train_path), transform=train_transformer
        ),
    ...
    query_loader = DataLoader(
        Preprocessor(
            dataset.query, root=osp.join(dataset.images_dir, dataset.query_path), transform=test_transformer
        ),
        batch_size=batch_size,
        num_workers=workers,
        shuffle=False,
    ...
    if camstyle <= 0:
        ...
        camstyle_loader = DataLoader(
            Preprocessor(
                dataset.camstyle,
                root=osp.join(dataset.images_dir, dataset.camstyle_path),
...
def main(args):
    ...
    # Create data loaders
    dataset, num_classes, train_loader, query_loader, gallery_loader, camstyle_loader = get_data(
        args.dataset,
        args.data_dir,
        args.height,
        args.width,
    ...
    if args.evaluate:
        print("Test:")
        evaluator.evaluate(
            query_loader, gallery_loader, dataset.query, dataset.gallery, args.output_feature, args.rerank
        )
        return
    ...
    optimizer = torch.optim.SGD(
        param_groups, lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay, nesterov=True
    )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        save_checkpoint(
            {'state_dict': model.module.state_dict(), 'epoch': epoch + 1},
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="learning rate of new parameters, for pretrained " "parameters it is 10 times smaller than this",
    )

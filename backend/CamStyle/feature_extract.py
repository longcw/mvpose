...
def pairwise_affinity(query_features, gallery_features, query=None, gallery=None):
    ...
    affinity = torch.sigmoid(normalized_affinity * torch.tensor(5.0))  # x5 to match 1->1
    # pro = x @ y.t ()
...
class FeatureExtractor(object):
    def __init__(self, model=None):
        ...
        if model:
            ...
            model = models.create(
                self.args['arch'],
                num_features=self.args['features'],
                dropout=self.args['dropout'],
                num_classes=751,
            )  # 751 is where the number of classes at market 1501

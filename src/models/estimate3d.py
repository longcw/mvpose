...
import torch
import numpy as np
from src.m_utils.geometry import (
    geometry_affinity,
...
class MultiEstimator(object):
    ...
    def _infer_single2d(self, imgs, img_id=0, dir='/home/jiangwen/tmp/Multi'):
        ...
        for cam_id, img in enumerate(imgs):
            ...
            for person_id, result in enumerate(results):
                ...
                this_info_dict[img_id][person_id]['cropped_img'] = cv2.cvtColor(
    ...
    def _estimate3d(self, img_id, show=False, plt_id=0, rtn_2d=False):
        ...
        geo_affinity_mat = geometry_affinity(
            pose_mat.copy(), self.dataset.F.numpy(), self.dataset.dimGroup[img_id]
        )
        ...
        match_mat = matchSVT(
            W,
            dimGroup,
            alpha=self.cfg.alpha_SVT,
            _lambda=self.cfg.lambda_SVT,
        ...
        if self.cfg.hybrid:
            ...
            multi_pose3d, chosen_img = self._top_down_pose_kernel(
                geo_affinity_mat, matched_list, pose_mat, sub_imgid2cam
            )
        ...
        for person in matched_list:
            ...
            if self.cfg.reprojection_refine and len(person) > 2:
                for joint_idx in range(human_coco.shape[1]):
                    ...
                    ) / reprojected_error.std() < self.cfg.refine_threshold
    ...
    def _top_down_pose_kernel(self, geo_affinity_mat, matched_list, pose_mat, sub_imgid2cam):
        ...
        for person in matched_list:
            ...
            if self.cfg.use_bundle:
                pose3d_homo = bundle_adjustment(
                    pose3d_homo, person, self.dataset, pose_mat, sub_imgid2cam, logging=logger
                )
...
if __name__ == '__main__':
    ...
    for imgs in tqdm(test_loader):
        ...
        poses3d = est.predict(
            imgs=this_imgs, camera_parameter=test_camera_parameter, show=False, template_name='Shelf'
        )
        # print ( poses3d )

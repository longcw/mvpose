...
def show_epilines(img_cv0, img_cv1, F_01, pts_0, pts_1, show=True):
    ...
    img_draw_0, img_draw_1 = (
        cv2.cvtColor(img_draw_0, cv2.COLOR_BGR2RGB),
        cv2.cvtColor(img_draw_1, cv2.COLOR_BGR2RGB),
    )
...
def visualize(
    ...
    if det_box_list:
        for det_boxes in det_box_list:
            ...
            cv2.rectangle(im, (bb[0], bb[1]), (bb[0] + bb[2], bb[0] + bb[3]), (0, 0, 255), 5)
...
def draw_skeleton(aa, kp, show_skeleton_labels=False):
    ...
    for i, j in skeleton:
        if (
            ...
            and kp[j - 1][0] >= 0
            and kp[j - 1][1] >= 0
            and (len(kp[i - 1]) <= 2 or (len(kp[i - 1]) > 2 and kp[i - 1][2] > 0.1 and kp[j - 1][2] > 0.1))
        ):
...
def show_panel(
    ...
    for i, cam_id in enumerate(dataset.cam_names):
        ...
        img = visualize(img, keypoints_list=[i['pose2d'] for _, i in info_dict.items()], return_img=True)
...
def plot_shelf_multi_pose3d(poses, inplace=False):
    ...
    for i, pose in enumerate(poses):
        ...
        for j in range(pose.shape[1]):
            col = '#%02x%02x%02x' % joint_color(j)
            ax.scatter(pose[0, j], pose[1, j], pose[2, j], c=col, marker=maker[i % 2], edgecolor=col)
        smallest = pose.min()
        largest = pose.max()
        ax.set_xlim3d(smallest, largest)
        ax.set_ylim3d(smallest, largest)
...
def show_panel_mem(
    ...
    for i, cam_id in enumerate(dataset.cam_names):
        ...
        imgDeteced = visualize(
            img,
            keypoints_list=[i['pose2d'] for i in info_dict],
            det_box_list=[i['bbox'] for i in info_dict],
            return_img=True,
        )
        imgProjected = visualize(img, keypoints_list=reprojectedPoses[i], return_img=True)
...
def plot_panoptic(poses, x_poses=list(), inplace=True):
    ...
    for i, pose in enumerate(x_poses):
        ...
        for j in range(pose.shape[1]):
            col = '#%02x%02x%02x' % joint_color(j)
            ax.scatter(pose[0, j], pose[1, j], pose[2, j], c=col, marker='x', edgecolor=col)
        smallest = pose.min()
        largest = pose.max()
        ax.set_xlim3d(smallest, largest)
        ax.set_ylim3d(smallest, largest)
...
def plotPaperRows(
    ...
    for camIdx, cam_id in enumerate(dataset.cam_names):
        ...
        img = dataset.info_dict[cam_id]['image_data']
        imgDetected = visualizeSkeletonPaper(
            img,
            [(255, 255, 255) for _ in info_dict],
            keypoints_list=[i['pose2d'] for i in info_dict],
            det_box_list=[i['bbox'] for i in info_dict],
        )
        imgMatched = visualizeSkeletonPaper(
            img,
            colorAssignment,
            keypoints_list=[i['pose2d'] for i in info_dict],
            det_box_list=[i['bbox'] for i in info_dict],
        )
        imgProjected = visualizeSkeletonPaper(img, all_color, keypoints_list=reprojectedPoses[camIdx])
...
def visualizeSkeletonPaper(img, colors, det_box_list=None, keypoints_list=None):
    ...
    if det_box_list:
        for boxIdx, det_boxes in enumerate(det_box_list):
            det_boxes = np.array(det_boxes)
            bb = det_boxes[:4].astype(int)
            cv2.rectangle(im, (bb[0], bb[1]), (bb[0] + bb[2], bb[1] + bb[3]), colors[boxIdx], 5)

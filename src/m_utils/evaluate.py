...
def evaluate(model, actor3D, range_, loader, is_info_dicts=False, dump_dir=None):
    ...
    for idx, imgs in enumerate(tqdm(loader)):
        ...
        try:
            if is_info_dicts:
                ...
                model.dataset = MemDataset(
                    info_dict=info_dicts, camera_parameter=camera_parameter, template_name='Unified'
                )
                ...
                poses3d = model.predict(
                    imgs=this_imgs, camera_parameter=camera_parameter, template_name='Unified', show=False
                )
        ...
        for pid in range(len(actor3D)):
            ...
            for i, bone in enumerate(bones):
                ...
                if is_right(
                    model_pose[start_point], model_pose[end_point], gt_pose[start_point], gt_pose[end_point]
                ):
    ...
    tb = PrettyTable()
    tb.field_names = (
        ['Bone Group'] + [f'Actor {i}' for i in range(bone_person_wise_result['Head'].shape[0])] + ['Average']
    )
    ...
    if dump_dir:
        np.save(
            osp.join(
                dump_dir,
                time.strftime(str(model_cfg.testing_on) + "_%Y_%m_%d_%H_%M", time.localtime(time.time())),
            ),
            check_result,
        )
        with open(
            osp.join(
                dump_dir,
                time.strftime(str(model_cfg.testing_on) + "_%Y_%m_%d_%H_%M.csv", time.localtime(time.time())),
            ),
            'w',
        ) as f:
            writer = csv.writer(f)
...
if __name__ == '__main__':
    ...
    for dataset_idx, dataset_name in enumerate(args.datasets):
        ...
        if args.dumped_dir:
            ...
            test_loader = DataLoader(
                test_dataset, batch_size=1, pin_memory=True, num_workers=12, shuffle=False
            )

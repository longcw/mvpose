...
def projected_distance_origin(pts_0, pts_1, F):
    ...
    lines = cv2.computeCorrespondEpilines(
        pts_0.reshape(-1, 1, 2), 2, F
    )  # I know 2 is not seems right, but it actually work for this dataset
    ...
    for cam_id0, h in enumerate(range(len(dimGroup) - 1)):
        for cam_add, k in enumerate(range(cam_id0 + 1, len(dimGroup) - 1)):
            ...
            distance_matrix[dimGroup[k] : dimGroup[k + 1], dimGroup[h] : dimGroup[h + 1]] = distance_matrix[
                dimGroup[h] : dimGroup[h + 1], dimGroup[k] : dimGroup[k + 1]
            ].T
        ...
        for cam_id1, k in enumerate(range(len(dimGroup) - 1)):
            for i in range(dimGroup[h], dimGroup[h + 1]):
                for j in range(dimGroup[k], dimGroup[k + 1]):
                    distance_matrix[i, j] += (
                        projected_distance(points_set[i], points_set[j], Fs[cam_id0, cam_id1])
                        + projected_distance(points_set[j], points_set[i], Fs[cam_id1, cam_id0])
                    ) / 2
    affinity_matrix = -(distance_matrix - distance_matrix.mean()) / distance_matrix.std()
    # TODO: add flexible factor
    affinity_matrix = 1 / (1 + np.exp(-5 * affinity_matrix))
...
def multiTri(Ps, Ys):
    ...
    Xs_homo = torch.sum(Ps.transpose(2, 1) @ Ps, dim=0).inverse() @ torch.sum(
        Ps.transpose(2, 1) @ Ys_homo, dim=0
    )
